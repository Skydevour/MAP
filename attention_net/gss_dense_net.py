...
def _bn_relu_gss_conv3d(alpha, growth_rate):
    def f(input):
        activation = _bn_relu(input)
        g_filters = K.int_shape(input)[-1] // 2
        g1_inpt = Lambda(lambda z:z[:, :, :, :, :g_filters])(activation)
        g2_inpt = Lambda(lambda z:z[:, :, :, :, g_filters:])(activation)
        g1_filters = int((1.-alpha) * growth_rate)
        g2_filters = growth_rate - g1_filters
        g1 = Conv3D(filters=g1_filters, kernel_size=(3, 3, 1), strides=(1, 1, 1),
                    kernel_initializer="he_normal", padding="same", use_bias=False,
                    kernel_regularizer=l2(1e-4))(g1_inpt)
        g2 = Conv3D(filters=g2_filters, kernel_size=(3, 3, 3), strides=(1, 1, 1),
                    kernel_initializer="he_normal", padding="same", use_bias=False,
                    kernel_regularizer=l2(1e-4))(g2_inpt)
        x = Concatenate()([g1, g2])
        return x
    return f
...
def conv_block(x, growth_rate, channel_axis=-1):
    x1 = _bn_relu_gss_conv3d(0.125, growth_rate)(x)
...
def gssdNet40(input_shape, nb_class, nb_dense_block=3):
    channel_axis = 1 if K.image_data_format() == "channels_first" else -1
    filters = 16
