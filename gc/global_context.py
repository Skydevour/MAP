...
def global_context_block(ip, reduction_ratio=16, transform_activation='linear'):
    ...
    # [B, ***, C] or [B, C, ***]
    input_flat = _spatial_flattenND(ip, rank)          # (B, H, W, D, C) -> (B, HxWxD, C)      branch-1
    # [B, ..., C] or [B, C, ...]
    context = _convND(ip, rank, channels=1, kernel=1)  # (B, H, W, D, C) -> (B, H, W, D, 1)    branch-2
    # [B, ..., 1] or [B, 1, ...]
    context = _spatial_flattenND(context, rank)        # (B, H, W, D, 1) -> (B, HxWxD, 1)      branch-2
    # [B, ***, 1] or [B, 1, ***]
    context = Softmax(axis=flat_spatial_dim)(context)  # (B, HxWxD, 1) -> (B, HxWxD, 1)        branch-2

    # Compute context block outputs
    context = dot([input_flat, context], axes=flat_spatial_dim)  # (B, 1, C)
    # [B, C, 1]
    context = _spatial_expandND(context, rank) # (B, 1, 1, 1, C)
    # [B, C, 1...] or [B, 1..., C]
...
if __name__ == '__main__':
    ...
    ip = Input(shape=(224, 224, 91, 64))
    x = global_context_block(ip, reduction_ratio=16)

    model = Model(ip, x)
    plot_model(model, to_file="global_context.png")
